...
class Submit extends Base
{
     ...
     * @return string
     */
    public function input($title = '', $field = '',$type='text'){
        $html = '';
        $html .= '<label for="'.$field.'">'.$title.':</label>';
        $html .= '<input type="'.$type.'" class="layui-input" name="'.$field.'" id="'.$field.'" placeholder="'.$title.'" lay-verify="'.$field.'" autocomplete="off">';
        return $html;
    }
    ...
    public function captcha($title = '验证码', $field = 'verify'){
        ...
        $html .= '<img id="'.$field.'_img" src="'.captcha_src().'" alt="'.$title.'" class="captcha">';
        $html .= '</div>';
        return $html;
    }
